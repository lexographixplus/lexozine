...
export default function IssueDashboard() {
  ...
  return (
    <main className="dashboard-shell">
      ...
      <section className="dashboard-hero"><div><span className="eyebrow">Editorial operations</span><h1>Issues</h1><p>Manage each edition through writing, design, review and publication from one structured workflow.</p>{message ? <div className="publication-message">{message}</div> : null}</div><button className="dashboard-create" onClick={() => setShowTemplates((value) => !value)}><Plus size={17} /> New issue</button></section>
      {showTemplates ? <section className="template-picker"><div className="template-picker-heading"><LayoutTemplate size={17} /><strong>Start blank or choose a publication system</strong></div><div className="template-cards"><button onClick={() => void createBlank()} className="template-card template-blank"><div className="template-preview template-preview-blank"><span>LEXOZINE</span><strong>+</strong><small>Blank issue</small></div><div><strong>Blank issue</strong><p>Start with an empty issue and build the editorial structure yourself.</p><small>No articles, blocks, pages, cover layout or sample copy</small></div><ArrowRight size={16} /></button>{templateCatalog.map((template) => <button key={template.id} onClick={() => void createIssue(template.id)} className={`template-card template-${template.id}`}><div className="template-preview"><span>LEXOZINE</span><strong>Aa</strong><small>{template.name}</small></div><div><strong>{template.name}</strong><p>{template.description}</p><small>Next: organise articles and assign editable layouts</small></div><ArrowRight size={16} /></button>)}</div></section> : null}
      ...
      <section className="issue-grid">
        ...
        <button className="new-issue-placeholder" onClick={() => setShowTemplates(true)}><Plus size={24} /><strong>Create another issue</strong><span>Start blank or choose a reusable publication template</span></button>
      </section>
    </main>
  );
}
