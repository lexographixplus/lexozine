...
export default function PrintEdition({ issue }: { issue: Issue }) {
  ...
  return <main className="print-edition" style={{"--print-paper":theme.paper,"--print-ink":theme.ink,"--print-accent":theme.accent} as CSSProperties}>
    <section className="print-page print-cover">
      {issue.coverImageUrl ? <div className="print-cover-photo" style={{backgroundImage:`linear-gradient(180deg,rgba(0,0,0,.08),rgba(0,0,0,.62)),url(${issue.coverImageUrl})`}}/>:<div className="print-cover-art"/>}
      <div className="print-cover-top"><span>ISSUE {issue.number}</span><span>{issue.editionDate}</span></div><div className="print-masthead">LEXOZINE</div><div className="print-cover-copy"><span>{issue.title}</span><h1>{issue.articles[0]?.title??issue.title}</h1><p>{issue.description}</p></div><div className="print-coverlines">{issue.coverLines.map((line)=><span key={line}>{line}</span>)}</div>
    </section>
