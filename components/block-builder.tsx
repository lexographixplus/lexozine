...
export default function BlockBuilder() {
  ...
  useEffect(()=>{
    ...
    async function load() {
      try {
        ...
        if (found && alive) { setIssue(found); setArticleId(found.articles[0]?.id ?? ""); }
      } catch {
        if (alive) setStatus("Issue data is temporarily unavailable");
      }
    }
    void load();
    return () => { alive = false; };
  ...
  async function replaceBlocks(nextBlocks: StoryBlock[], message: string){
    if(!article)return;
    ...
    try { await persist(next); setStatus(message); }
    catch { setStatus("Changes could not be saved. Try again."); }
  }
  ...
  function addBlock(type: Extract<BlockType,"headline"|"deck"|"body"|"pullquote">, text:string){
    ...
    void replaceBlocks([...ordered,block],`${type} block added to ${article.title}`);
  }
