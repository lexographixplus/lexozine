...
import { ArrowDown, ArrowLeft, ArrowRight, BookOpen, FileDown, Link2, Menu, Minus, Moon, Plus, Share2, SlidersHorizontal, Sun, X } from "lucide-react";
...
import FixedLayoutEdition from "@/components/fixed-layout-edition";
...
export default function DigitalEdition({ initialIssue }: { initialIssue?: Issue }) {
  ...
  const [readerView, setReaderView] = useState<"magazine" | "read">(() => issue.fixedLayout ? "magazine" : "read");

  if (readerView === "magazine" && issue.fixedLayout) {
    return <FixedLayoutEdition issue={issue} edition={issue.fixedLayout} onRead={() => setReaderView("read")} />;
  }

  return <ReflowDigitalEdition initialIssue={issue} onOpenMagazine={issue.fixedLayout ? () => setReaderView("magazine") : undefined} />;
}

function ReflowDigitalEdition({ initialIssue, onOpenMagazine }: { initialIssue?: Issue; onOpenMagazine?: () => void }) {
  const [issue] = useState<Issue>(() => initialIssue ?? createIssueTemplate("editorial"));
  ...
  const publicPdfHref = issue.status === "published" && issue.publicSlug && (issue.visibility === "public" || issue.visibility === "unlisted")
    ? `/api/publications/${encodeURIComponent(issue.publicSlug)}/pdf`
    : null;
  ...
  return <main className={`edition-shell edition-mode-${readerMode} edition-theme-${issue.theme} edition-experience-${readerExperience}`} style={readerStyle}>
    ...
    <header className="edition-nav"><button className="edition-menu" onClick={() => { setNavOpen((value) => !value); setSettingsOpen(false); }} aria-label={navOpen ? "Close contents" : "Open contents"} aria-expanded={navOpen}><Menu size={18}/></button><div className="edition-brand">{cover.masthead}</div><div className="edition-meta">ISSUE {issue.number} · {issue.editionDate.toUpperCase()} · {totalReadingTime || "—"} MIN READ</div><div className="edition-reader-actions">{onOpenMagazine ? <button className="edition-experience-toggle" onClick={onOpenMagazine} aria-label="Open fixed-layout magazine"><span>Magazine</span></button> : null}<button className="edition-experience-toggle" onClick={() => setReaderExperience((value) => value === "paged" ? "continuous" : "paged")} aria-label={readerExperience === "paged" ? "Use continuous reading" : "Use page-by-page reading"}><span>{readerExperience === "paged" ? "Pages" : "Scroll"}</span></button><button className="edition-reader-control" onClick={() => setFontStep((value) => Math.max(-1, value - 1))} aria-label="Decrease text size" disabled={fontStep <= -1}><Minus size={14}/></button><button className="edition-reader-control" onClick={() => setFontStep((value) => Math.min(2, value + 1))} aria-label="Increase text size" disabled={fontStep >= 2}><Plus size={14}/></button><button className="edition-reader-control" onClick={() => setReaderMode((value) => value === "night" ? "paper" : "night")} aria-label={readerMode === "night" ? "Use light reading mode" : "Use night reading mode"}>{readerMode === "night" ? <Sun size={14}/> : <Moon size={14}/>}</button><button className="edition-settings-trigger" onClick={() => { setSettingsOpen((value) => !value); setNavOpen(false); }} aria-label="Reader settings" aria-expanded={settingsOpen}><SlidersHorizontal size={15}/></button>{publicPdfHref ? <a className="edition-pdf-trigger" href={publicPdfHref} aria-label="Download PDF edition"><FileDown size={15}/><span>PDF</span></a> : null}<button className="edition-share" onClick={() => void shareIssue()}><Share2 size={16}/><span>{shareMessage || "Share"}</span></button></div></header>
    ...
    <section className={readerPageClass(0, `edition-cover ${importedCover ? "edition-cover-imported" : "edition-cover-generated"}`)}>{coverImageUrl ? <div className="edition-cover-art has-photo" style={coverArtStyle}/> : <div className="edition-cover-art"/>}{!importedCover ? <><div className="edition-cover-overlay" style={overlayStyle}/><div className="edition-cover-copy" style={{ textAlign: cover.textAlign }}><span className="edition-kicker">{issue.title} · Issue {issue.number}</span><h1>{cover.mainHeadline}</h1><div>{cover.deck}</div><div className="edition-cover-actions"><a href={beginHref} onClick={(event) => { if (readerExperience === "paged") { event.preventDefault(); goToPage(1); } }} className="edition-read">Begin reading <ArrowDown size={17}/></a>{publicPdfHref ? <a className="edition-pdf-read" href={publicPdfHref}><FileDown size={16}/> Download PDF</a> : null}</div></div><div className="edition-coverlines">{cover.lines.map((line) => <span key={line}>{line}</span>)}</div></> : null}</section>
